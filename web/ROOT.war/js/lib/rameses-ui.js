...
function createSideMenu() {
	...
	if (tdnodes)
	{
		for (var i=0; i<tdnodes.length; i++) {
			...
			var aNode = $(elem.find('div.menuitem a')); 
			if (aNode.attr('href') == window.location.hash) 
			{
				divNode.addClass('selected');
			} 
			aNode.click(function(e) { 
...
function PopupOpener( id, params, options ) 
{
    ...
    this.load = function() {
		...
		function createDialog() {
			...
			options.close = function(e) { dialogAdapter('CLOSED',e); }
			options.beforeClose = function(e) { return dialogAdapter('BEFORE_CLOSE',e); }; 
            div.dialog(options);
			BindingUtils.load(div);
			
			var parentElem = div.offsetParent();
			if (parentElem) {
				var jparentElem = $(parentElem);
				jparentElem.addClass('rui-theme-background rui-popupopener'); 
				
				var uidc = jparentElem.find('.ui-dialog-titlebar')[0]; 
				if (uidc) $(uidc).addClass('rui-theme-header-background');
				
				uidc = jparentElem.find('.ui-dialog-content')[0]; 
				if (uidc) $(uidc).addClass('rui-popupopener-body');
			} 
		} 
